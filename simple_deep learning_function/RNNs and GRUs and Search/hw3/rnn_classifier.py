    ...
    def forward(self, x, h_0=None):
        ...
        for i in range(seq_len):
            ...
            for j in range(self.num_layers):
                h_prev_t = self.rnn[j](h_prev_t, self.hiddens[i][j])
                hidden[j] = h_prev_t
        ...
        * Normalize dh by batch_size since initial hidden states are also treated
        ...
        for i in range(seq_len - 1, -1, -1):
            for j in range(self.num_layers - 1, -1, -1):
                if j > 0:
                    dx, dh_prev_t = self.rnn[j].backward(dh[j], self.hiddens[i + 1][j],
                                                         self.hiddens[i + 1][j - 1], self.hiddens[i][j])
                    dh[j] = dh_prev_t
                    dh[j - 1] += dx
                else:
                    dx, dh_prev_t = self.rnn[j].backward(dh[j], self.hiddens[i + 1][j],
                                                         self.x[:, i, :], self.hiddens[i][j])
                    dh[j] = dh_prev_t
