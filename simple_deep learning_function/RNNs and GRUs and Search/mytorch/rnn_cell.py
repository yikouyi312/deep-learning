...
class RNNCell(object):
    ...
    def forward(self, x, h_prev_t):
        ...
        """
        h_t = self.activation(np.tensordot(x, self.W_ih, axes=([1], [1]))
                              + np.expand_dims(self.b_ih, axis=0)
                              + np.tensordot(h_prev_t, self.W_hh, axes=([1], [1]))
                              + np.expand_dims(self.b_hh, axis=0))# TODO
    ...
    def backward(self, delta, h_t, h_prev_l, h_prev_t):
        ...
        dz = self.activation.backward(h_t) * delta # TODO
        # 1) Compute the averaged gradients of the weights and biases
        self.dW_ih += np.transpose(np.tensordot(h_prev_l, dz, axes=([0], [0]))) / batch_size # TODO
        self.dW_hh += np.transpose(np.tensordot(h_prev_t, dz, axes=([0], [0]))) / batch_size # TODO
        self.db_ih += np.sum(dz, axis=0) / batch_size # TODO
        self.db_hh += np.sum(dz, axis=0) / batch_size# TODO

        # # 2) Compute dx, dh_prev_t
        dx        = np.tensordot(dz, self.W_ih, axes=1)# TODO
        dh_prev_t = np.tensordot(dz, self.W_hh, axes=1)# TODO
