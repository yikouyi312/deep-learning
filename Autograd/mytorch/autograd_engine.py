...
class Autograd:
    ...
    def add_operation(self, inputs, output, gradients_to_update, backward_operation):
        """
            ...
            Args:
                ...
                - gradients_to_update: parameter gradients if for parameter of
                            network or None (numpy.ndarray, None)
                    Given the linear layer as shown in the writeup section
    ...
    def backward(self, divergence):
        ...
        operation = self.operation_list[-1]
        self.memory_buffer.add_spot(operation.output)
        self.memory_buffer.set_param(operation.output, divergence)
        ...
        for operation in reversed(self.operation_list):
            ...
            if len(operation.inputs) == 3:
                a_grad, b_grad, c_grad = operation.backward_operation(self.memory_buffer.get_param(operation.output),
                                                              operation.inputs[0],
                                                              operation.inputs[1], operation.inputs[2])
                self.memory_buffer.update_param(operation.inputs[0], a_grad)
                self.memory_buffer.update_param(operation.inputs[1], b_grad)
                self.memory_buffer.update_param(operation.inputs[2], c_grad)
                if operation.gradients_to_update[0] is not None:
                    operation.gradients_to_update[0] += a_grad
                if operation.gradients_to_update[1] is not None:
                    operation.gradients_to_update[1] += b_grad
                if operation.gradients_to_update[2] is not None:
                    operation.gradients_to_update[2] += c_grad
            elif len(operation.inputs) == 2:
                a_grad, b_grad = operation.backward_operation(self.memory_buffer.get_param(operation.output),
                                                              operation.inputs[0],
                                                              operation.inputs[1])
                self.memory_buffer.update_param(operation.inputs[0], a_grad)
                self.memory_buffer.update_param(operation.inputs[1], b_grad)
                ...
                a_grad = operation.backward_operation(self.memory_buffer.get_param(operation.output),
                                                      operation.inputs[0])
                self.memory_buffer.update_param(operation.inputs[0], a_grad)
                if operation.gradients_to_update[0] is not None:
                    operation.gradients_to_update[0] += a_grad
