...
from cnn import CNN, get_training_stats
...
def visualize(outpath):
    ...
    activations = [nn.Tanh(autograd), nn.ReLU(autograd), nn.Sigmoid(autograd)]
    criterion = nn.loss.SoftmaxCrossEntropy(autograd)
    lr = 0.001

    cnn = CNN(input_width, num_input_channels, num_channels, kernel_sizes,
              strides, num_linear_neurons, activations, criterion, lr,
              autograd)

    visualize_training_statistics(cnn, dset, epochs, batch_size, savepath)
...
def visualize_training_statistics(cnn,
                                  dset,
                                  epochs,
                                  batch_size,
                                  savepath=None):
    ...
    plotline(validation_losses, "Epoch", "Loss", "Validation Loss",
             os.path.join(path, "val_loss.png"))
    plotline(validation_errors, "Epoch", "Error", "Validation Error",
             os.path.join(path, "val_error.png"))


def parse_args():
    parser = argparse.ArgumentParser(description="11-785 HW2P1 Visualizer")
