...
class BatchNorm1d:
    ...
    def forward(self, Z, eval=False):
        ...
        self.NZ = (Z - ones @ self.M) / (
                    ones @ np.sqrt(self.V + self.eps))  # TODO (NxC - Nx1 @ 1xC) / (Nx1@(1xC+1))=NxC
    ...
    def backward(self, dLdBZ):
        ...
        dLdV = -0.5 * np.sum(dLdNZ * ((self.Z - ones @ self.M) * (ones @ np.power(self.V + self.eps, -1.5))),
                             axis=0)  # TODO 1xC
        dLdM = -np.sum(dLdNZ * np.power(self.V + self.eps, -0.5), axis=0) - 2 / self.N * dLdV * np.sum(self.Z - self.M,
                                                                                                       axis=0)  # TODO 1xC

        dLdZ = dLdNZ * np.power(self.V + self.eps, -0.5) + dLdV * (
                    2 / self.N * (self.Z - self.M)) + dLdM / self.N  # TODO 1xC * 1xC + 1xC* 1xC
