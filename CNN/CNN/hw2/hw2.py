...
class CNN(object):
    ...
    def __init__(self, input_width, num_input_channels, num_channels, kernel_sizes, strides,
        ...
        self.convolutional_layers = [Conv1d(in_channels=num_input_channels,
        ...
        in_features = (input_width - kernel_sizes[0])//strides[0] + 1
        #print(in_features)
        for i in range(1, len(num_channels)):
            in_features = (in_features - kernel_sizes[i]) // strides[i] + 1
            #print(in_features)
        self.linear_layer = Linear(in_features * num_channels[-1], num_linear_neurons)
    ...
    def forward(self, A):
        ...
        self.Z = A
        self.Z = self.convolutional_layers[0].forward(self.Z)
        self.Z = self.activations[0].forward(self.Z)
        self.Z = self.convolutional_layers[1].forward(self.Z)
        self.Z = self.activations[1].forward(self.Z)
        self.Z = self.convolutional_layers[2].forward(self.Z)
        self.Z = self.activations[2].forward(self.Z)
    ...
    def backward(self, labels):
        ...
        grad = grad * self.activations[2].backward()
        grad = self.convolutional_layers[2].backward(grad)
        grad = grad * self.activations[1].backward()
        grad = self.convolutional_layers[1].backward(grad)
        grad = grad * self.activations[0].backward()
